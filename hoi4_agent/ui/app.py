...
from hoi4_agent.config.settings import Config, load_config
...
def main():
    ...
    st.caption(
        f"v4 · **{provider_label}** · {len(ctx.countries)}개 국가 · "
        f"{len(ctx.characters)}개 캐릭터 · {tool_total}개 도구"
    )

    if config.ai_provider == "anthropic" and not config.anthropic_key:
